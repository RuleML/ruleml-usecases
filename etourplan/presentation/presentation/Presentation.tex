...
\color{blue} \item University of NewBrunswick (UNB) \vspace*{0.2cm} %\item References \vspace*{0.3cm} 
%\end{columns}
\end{itemize}
}
...
\vspace{0.2cm}
\pause
\item To evaluate the overall operation of the eTourPlan prototype as run in the \textbf{OO jDREW} reasoning engine prototype
\end{itemize}
}
...
\vspace{0.3cm}
\end{itemize}
\color {red} \large \textbf{A ``need" for providing trip planning options along with recommendations}
...
\begin{itemize}
\item Object-centric facts of tourist entities
\end{itemize}
\vspace{0.3cm}
\textbf{\color{blue}Rules:}
\begin{itemize}
\item Search facility more precise than with keywords (Semantic search of knowledge)
...
\begin{figure}[h] 
\includegraphics[width=10cm, height=7.5cm]{architecture}
\end{figure}
}
...
\frame{\frametitle{3.2 Ontology Design}
 ...
 \item Interoperabilty among many agents and applications
...
\frame{\frametitle{3.3.5 FOAF-like Profile of Thimphu\_Tshechu}
\begin{figure} [tbph]
\centering
\scriptsize
\begin{tabular}{|l|}
...
\begin{itemize}
\item Provincial details
\item Activity details (Events and Attractions)
\item Accommodation details
\end{itemize}
\vspace{0.2cm}
...
\end{itemize}
\vspace{0.2cm}
\item \textbf{\color{red}Rule System for eTourPlan Travel Planner}
\begin{itemize}
...
\begin{itemize}
\item {\color{blue}Partonomy} is a classification system based on the {``\color{blue}partOf"} relation between subparts and superparts \vspace{0.3cm}
...
\item Richer representation of knowledge 
\item Avoids information ambiguity
\end{itemize}
\vspace{0.3cm}
\item {\color{blue}General partonomy} rule
\begin{itemize}
\item A generic definition of the ``partOf" relation
\item Transitive closure of the binary ``partOf" relation 
\end{itemize}
\end{itemize}
}
...
\begin{columns}[c]
 \column{.56\textwidth}
\begin{figure}[h] 
\includegraphics[width=5.5cm, height=5.1cm]{graph1.jpg}
...
\hline
 \textbf{\color{blue}Sample Query:}\\
   getRouteDetails(startPoint-$>$A; endPoint-$>$H; {\color{red}?AllRoutes}, {\color{red}?ShortestRoute}).\\
 \\
 \newline
 \textbf{\color{blue}OO jDREW TD Result:}\\

  ?AllRoutes= [[[A, E, G, H], 10.0:Real], \\ \newline 
   \hspace{0.48in}   [[A, B, E, G, H], 13.0:Real], \\  \newline
    \hspace{0.48in}  [[A, E, F, G, H], 10.0 : Real], \\ \newline 
   \hspace{0.48in} [[A, B, C, E, G, H], 20.0 : Real],\\  \newline 
		\hspace{0.48in} [[A, B, E, F, G, H], 13.0 : Real], \\ \newline 
		\hspace{0.48in} [[A, B, C, D, F, G, H], 16.0 : Real],\\ \newline 
		\hspace{0.48in} [[A, B, C, E, F, G, H], 20.0 : Real], \\ \newline 
		\hspace{0.48in} [[A, E, C, D, F, G, H], 16.0 : Real], \\ \newline 
		\hspace{0.48in} [[A, B, C, D, F, E, G, H], 20.0 : Real],\\ \newline 
		\hspace{0.48in}  [[A, B, E, C, D, F, G, H], 19.0 : Real], \\ \newline 
		\hspace{0.48in} [[A, E, B, C, D, F, G, H], 17.0 : Real]]\\ 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{4.3.1 Rule System for eTourPlan Travel Planner}
\frame{\frametitle{4.3.1 Rule System for eTourPlan Attraction-only Planning}
\begin{columns}[c]
 \column{.58\textwidth}
{\textbf{\color{blue} The planner performs the following steps}}:
 \vspace{0.1in}
 \pause
\begin{enumerate}
\item From the user-specified starting point, select the next
      related attraction.
 \vspace{0.2in}   
 \pause  
\item Compute and validate route and bus hours constrained
      by global constants:
\vspace{0.1in}
{\footnotesize \hspace{0.05in}- maxActivitySightSeeingHoursInADay(12:Real)}\\
{\footnotesize \hspace{0.04in}- maxHoursAtAnAttractionSite(4:Real)}\\

\vspace{0.2in}
\pause
\item On successful validation of distance and remaining time,
     detailed information of the selected attraction is 
     added to the travel plan.\\
\end{enumerate}
\column{.72\textwidth}
\begin{figure}
%\raggedright
...
\begin{columns}[c]
\column{.55\textwidth}
{\color{blue}The planner performs the following steps}:
 \vspace{0.2in}
\footnotesize
\begin{enumerate}
\pause
...
\pause
\vspace{0.2in}
\item On successful validation of the two above steps,\\ 
      detailed information of the selected event is added
      to the travel plan.
\vspace{0.2in}
\pause     
\item Attractions located in the subblock of the
      selected event location are recommended.
\pause      
\vspace{0.2in}     
\item On-route attraction recommendation is provided if the 
      user checks the option (constrained by the global 
      constant ``maxTimeGapBetweenEvents".     
\end{enumerate}
 \column{.75\textwidth}
\begin{figure}
...
\subsection{5.1 Search Operations}
\frame{\frametitle{5.1.1 Search Operations}
%\textbf{\color{blue}5.1. Search Operations:}\vspace{0.2cm}
\pause
\begin{enumerate}
\item Search for Provincial details
 \begin{itemize}
...
\vspace{0.1cm}
\pause
\item Search for Routes Details 
 \begin{itemize}
...
\end{itemize}
at {\color{blue} {\footnotesize(Subblock, Block, Province, Region, Country)}} of Partonomy
\vspace{0.2cm}
\pause
...
\frame{\frametitle{5.1.3 Search Results}
\begin{table} [tbph]
...
2&typeOfPlanning & eTourPlan(typeOfPlanning-$>${\color{blue}\textbf{EventCentric}};\\
  ...
  &eventNum      &$~~~$eventNum-$>${\color{blue}\textbf{2:Integer}}]; \\
  &                      &$~${\color{red}$?$TravelResult})\\         
\hline
\end{tabular} 
\end{table}
...
\frame{\frametitle{5.3.3 Detailed Result of a Travel Plan (Option 1)}
\begin{table} [tbph]
...
\frame{\frametitle{5.3.3 Detailed Result of a Travel Plan (Cont'd)}
\begin{table} [tbph]
\footnotesize
...
% \textbf{Values}& \\
 %\hline 
 		{\color{red}$?$TravelResult}&$~~~$\cellcolor{yellow}[EventName-$>$Tangbi\_Mani:Traditional\_festival;\\
...
\frame{\frametitle{5.3.4 Travel Planning Scenario}

{\large \emph{{\color{blue}User queries for an event-centric plan of {\color{red}3 events} between the {\color{red}1st of October to the 10th of November} and specifies a {\color{red}``maxBreak"} of {\color{red}10 days} and {\color{red}``minBreak"} of {\color{red}0 day} between main events. User also specifies the starting province,`` {\color{red}Paro:Province}"  and the final destination province as ``{\color{red}Thimphu:Province}". User checks {\color{red}``No"} for on-route attraction recommendation knowing that the planner provides recommendation of attractions at the event subblock.}}}
}

\frame{\frametitle{Travel Plan -At the starting point}
\begin{figure}
\centering
...
\frame{\frametitle{Travel Plan -Event 1 found}
\begin{figure}
\centering
\includegraphics[width=11.5cm, height=7cm]{E2}
\caption{Travel Planning Scenario}
\end{figure}}
\frame{\frametitle{Travel Plan -Event 2 found}
\begin{figure}
\centering
\includegraphics[width=11.5cm, height=7cm]{E3}
\caption{Travel Planning Scenario}
\end{figure}}
\frame{\frametitle{Travel Plan -Event 3 found}
\begin{figure}
\centering
\includegraphics[width=11.5cm, height=7cm]{E4}
\caption{Travel Planning Scenario}
\end{figure}}
\frame{\frametitle{Travel Plan -Return route to the end point}
\begin{figure}
\centering
...
\frame{\frametitle{Travel Planning -On-route attraction recommendation (Optional)}
\begin{figure}
\centering
...
\frame{\frametitle{5.3.5 Resulting Multiple Travel Plans}
\begin{table} [tbph]
\footnotesize
...
\begin{itemize}
\item \color{blue}Test environment:
\begin{itemize}
...
\begin{itemize}
\item\color{blue} Low execution times for retrieving subdomain information:
\begin{itemize}
...
\begin{itemize}
\item\color{blue}High execution times for recursive search predicates:
\begin{itemize}
\vspace{0.2in}
\item Integration of recursive predicates such as ``getAllAttractions", ``getAllEvents", and ``getAllAccommodations"  \vspace{0.2in}
 %Another source of inefficiency is from 
\item The textual order between rules is not exploited by our pure logic programs (simulate breadth-first parallel execution with exhaustive iterative deepening)  %thereby performing unnecessary searches for candidate bindings. 
\vspace{0.2in}
...
\item \vspace{0.2cm} Realized rule subsystems for various tourist services
 ...
 \end{itemize}
 \pause
 \item \vspace{0.2cm} Step-wise improvement of modelling an ``executable specification" of the eTourPlan prototype
 \pause
 \item \vspace{0.2in} eTourPlan KB (115  classes, 73 facts, and 37 rules) is tested and evaluated in the
...
\item \vspace{0.2cm} Our ``executable specification" can be integrated with a database and
                     could be translated to an independent database application
\item \vspace{0.2cm} A user-friendly GUI would complement the utility of the key operations of the eTourPlan prototype
\item \vspace{0.2in} The semantic modeling and search aspects of eTourPlan can be enhanced into a
                     semantic portal for Bhutan
\end{itemize}
}
